...
def build_sequences(df, feature_cols, window=32, predict_horizon=1, recency_tau=None, vol_alpha=0.5):
    ...
    for i in range(N):
        ...
        sample_vol[i] = vols[i+window-1]

    if recency_tau is None:
        recency_tau = window * 5.0
    idx = np.arange(N)
    recency_weight = np.exp((idx - (N-1)) / recency_tau)
    vol_norm = (sample_vol - sample_vol.min()) / (sample_vol.ptp() + 1e-12)
    vol_weight = 1.0 + vol_alpha * vol_norm
