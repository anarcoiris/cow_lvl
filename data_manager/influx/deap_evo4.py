...
def save_result_influx(influx_url, influx_token, influx_org, influx_bucket,
    ...
    try:
        print(f"[INFO] Guardando resultado en InfluxDB para símbolo: {symbol}...")
        client = InfluxDBClient(url=influx_url, token=influx_token, org=influx_org)
        write_api = client.write_api()

        p = (
            Point("deap_lstm_results")
            .tag("symbol", symbol)
        ...
        try:
            ...
            pass
        client.close()
        print("[OK] Resultado guardado correctamente en InfluxDB.")
        return True

    except Exception as e:
        print(f"[WARN] No se pudo guardar en InfluxDB ({e}). Guardando en CSV local...")
        fallback = {
        ...
        print("[OK] Resultado guardado en CSV fallback.")
        return False
...
def main():
    ...
    logging.info(f"Cargando datos desde InfluxDB...")
    logging.info(f"Datos cargados: {len(df)} filas")
    logging.info(f"Gen {gen} | avg={record['avg']:.6f} min={record['min']:.6f} ...")
